...
function Output(params) {
  ...
  if (params && JSUtil.isValidJSON(params)) {
    return Output.fromJSON(params);
  } else if (_.isObject(params)) {
    this.satoshis = params.satoshis;
    if (params.script) {
      this.setScript(params.script);
    }
  }
}
...
Output.prototype.toObject = function toObject() {
  ...
  if (this.script) {
    obj.script = this.script.toString();
  } else {
...
Output.prototype.setScript = function(script) {
  if (script instanceof Script) {
    ...
    var scriptBuffer;
    try {
      scriptInstance = Script.fromString(script);
      scriptBuffer = scriptInstance.toBuffer();
    } catch(e) {
      if(e instanceof errors.Script.InvalidBuffer) {
        scriptInstance = null;
        scriptBuffer = new Buffer(script, 'hex');
      } else {
        throw e;
      }
    }
    this._script = scriptInstance;
    this._scriptBuffer = scriptBuffer;
